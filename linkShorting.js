...
const getError = async (req, res) => {
    ...
    try {
        const response = await axios.post(GOOGLE_SAFE_BROWSING_URL, {
            client: {
                clientId: process.env.GOOGLE_SAFE_CLIENT_ID,
                clientVersion: "1.5.2"
            },
            threatInfo: {
    ...
    try {
        const SQLGETVALID = `SELECT CREATED_LINK FROM links WHERE LINK = '${longLink}'`;
        const haveLink = await pool.query(SQLGETVALID)
        ...
        const SQLISHAVETHIS = `SELECT * FROM links WHERE CREATED_LINK = '${completeLink}'`;
        const haveLikeLink = await pool.query(SQLISHAVETHIS)
        ...
        const SQLINSERTLINK = `INSERT INTO links (LINK, CREATED_LINK, USEFOR) VALUES ('${longLink}', '${completeLink}', '${0}')`;
        const insert = await pool.query(SQLINSERTLINK)
...
const redirectLink = async (req, res) => {
    ...
    try {
        const results = await pool.query('SELECT * FROM links WHERE CREATED_LINK = ?', [link]);
...
const getLINK = async (req, res) => {
    ...
    try {
        if (results.rows.length > 0) {
            ...
            const SQLCOUNT = 'UPDATE links SET USEFOR = ?, LAST_VIEW = ? WHERE CREATED_LINK = ?';

            await pool.query(SQLCOUNT, [linkData.USEFOR + 1, useDate.toLocaleString() , link]);
            const SQLCOUNTER = 'SELECT * FROM settings WHERE NAME IN (?, ?, ?)';
            const [settings] = await pool.query(SQLCOUNTER, ['COUNT_DAY', 'COUNT_WEEK', 'COUNT_YEAR']);

            if (settings.rows.length === 3) {
                const SQLINSERTCOUNT = 'UPDATE settings SET value = ? WHERE NAME = ?';
